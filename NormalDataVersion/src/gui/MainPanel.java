...
public class MainPanel extends JPanel {
	...
	public MainPanel(ArrayList[] routes, int[][] coordinates, int type) {
		this.routes = routes;
	...
	public void createGUI(Graphics g) {
		if (type == 0) {
			...
			createClusteredGUI(g);
		}
	}

	private void createClusteredGUI(Graphics g) {
		
		Color[] colors = {Color.BLUE, Color.CYAN, Color.DARK_GRAY, Color.GRAY, Color.GREEN,Color.LIGHT_GRAY, Color.MAGENTA, Color.ORANGE, Color.PINK, Color.RED, Color.YELLOW, Color.BLACK};

		for (int i = 1; i < coordinates.length; i++) {
			createNode(g, coordinates[i][0] * ((800 / MX) % 100),coordinates[i][1] * ((600 / MY) % 100), i + "");
		}

		createDepot(g, coordinates[0][0] * ((800 / MX) % 100),
		...
		for (int i = 0; i < routes.length; i++) {
			for (int j = 0; j < routes[i].size(); j++) {				
				createColoredNode(g,
						...
						coordinates[(int) routes[i].get(j)][1] * ((600 / MY) % 100), (int) routes[i].get(j) + "",colors[i]);
			}			
		}		
		
	}

	private void createColoredNode(Graphics g, int x, int y, String num,Color color) {
		g.setColor(color);
		g.fillOval(x - RADIUS, y - RADIUS, 2 * RADIUS, 2 * RADIUS);

		g.setColor(Color.WHITE);
		FontMetrics fm = new JLabel().getFontMetrics(new JLabel().getFont());
		g.drawString(num, x - fm.stringWidth(num) / 2, y
	...
	private void createRoutedGUI(Graphics g) {
		for (int i = 0; i < routes.length; i++) {
			for (int j = 0; j < (routes[i].size() - 1); j++) {
				if (j == 0) {
					...
					// First edge
					createEdge(g, coordinates[(int) routes[i].get(j)][0]
							* ((800 / MX) % 100),
							coordinates[(int) routes[i].get(j)][1]
									* ((600 / MY) % 100),
							coordinates[(int) routes[i].get(j + 1)][0]
									* ((800 / MX) % 100),
							coordinates[(int) routes[i].get(j + 1)][1]
									* ((600 / MY) % 100));
				} else {
					createEdge(g, coordinates[(int) routes[i].get(j)][0]
							* ((800 / MX) % 100),
			...
			if (routes[i].size() != 0) {
				createEndEdge(
						g,
						coordinates[(int) routes[i].get(routes[i].size() - 1)][0]
								* ((800 / MX) % 100),
						coordinates[(int) routes[i].get(routes[i].size() - 1)][1]
								* ((600 / MY) % 100), coordinates[0][0]
								* ((800 / MX) % 100), coordinates[0][1]
								* ((600 / MY) % 100), 5, 5);
			}
		}

		for (int i = 1; i < coordinates.length; i++) {
			createNode(g, coordinates[i][0] * ((800 / MX) % 100),
					coordinates[i][1] * ((600 / MY) % 100), i + "");
		}
	...
	private void createEndEdge(Graphics g, int x1, int y1, int x2, int y2,
			double al, double aw) {
		...
		length = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)); // I
																			// think,
																			// this
